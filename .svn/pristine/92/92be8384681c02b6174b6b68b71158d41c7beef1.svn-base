...
class RedEnvelopAPIClient {
    ...
    static func listGameLottery(ticket: String, completion:@escaping ([LobbyItemModel], String?)->Void) {
        Alamofire.request(RedEnvelopAPIRouter.listGame(ticket)).responseJSON { (responseData) in
            if responseData.result.value != nil {
                var lotteryItemModel: [LobbyItemModel] = []
                let jsonResponse = JSON(responseData.result.value!)
                let code = jsonResponse["code"].intValue
                let msg = jsonResponse["msg"].string
                ...
                if code == 1 {
                    let data = jsonResponse["data"].arrayValue
                    for lottJson in data {
                        let lotItem = LobbyItemModel(dictionary: lottJson, isLot: true)
                        lotteryItemModel.append(lotItem)
                        //print(lottJson)
                    }
                }
                
                completion(lotteryItemModel, msg ?? responseData.error?.localizedDescription)
            } else {
                completion([],responseData.error?.localizedDescription)
            }
        }
