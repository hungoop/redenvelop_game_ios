...
class RoomDetailViewController: BaseViewController, UpdateAfterBet {
    func updateBalance() {
        fetchUserInfo()
    }
    ...
    func setupNavigatorViews() {
        ...
        let rightItem2 = UIBarButtonItem(customView: reportBetButton)
    ...
    @objc func fetchUserInfo(showLoading: Bool = false) {
        ...
        UserAPIClient.userInfo(ticket: user.ticket) { [weak self] (userInfo, errorMessage) in
            ...
            if let `userInfo` = userInfo {
                ...
                self!.profileButton.setTitle("\(userInfo.allowcreditquota)", for: .normal)
            } else {
