...
class PackageHistoryLeftViewCell: UITableViewCell {
    ...
    func updateBullViews(bull: BullModel, isOpen: Bool = false, isKing: Bool = false, isBoomed: Bool = false, expired: Bool = false){
        ...
        var height2 =  bull.resultWagerInfo.count * 20
        
        //----banker----
        if(bull.myBanker() && resultWagers.count > 0){
            guard let packInfo = bull.bullInfo else {return}
            
            let label = UILabel().forAutolayout()
            label.font = UIFont.systemFont(ofSize: 12)
            
            label.text = String(format: "  %@ %@ %@ %@, %@ %@  ", AppText.bankerOwner, bull.round.banker, AppText.betPlace, packInfo.packettag, packInfo.packetamount < 0 ? AppText.betTotalLose : AppText.betTotalWin, packInfo.winnings.toFormatedString())
