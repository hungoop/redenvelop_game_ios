...
class UserAPIClient {
    ...
    static func appVersion(ticket: String, completion: @escaping (String?, String?) -> Void) {
        Alamofire.request(UserAPIRouter.appVersion(ticket)).responseJSON { (responseData) in
            if((responseData.result.value) != nil) {
                let jsonResponse = JSON(responseData.result.value!)
                let _ = jsonResponse["msg"].stringValue
                let code = jsonResponse["code"].intValue
                ...
                if code == 1 {
                    let data = jsonResponse["data"]
                    let ver_ios = data["ios"].string
                    let url_ios = data["ios_url"].string
                    completion(ver_ios, url_ios)
                } else {
                    completion(nil, nil)
                }
            } else {
